...
def bf16_required_version_check():
    ...
    if torch.distributed.is_nccl_available():
        if type(torch.cuda.nccl.version()) != tuple:
            ...
            if not (NCCL_MAJOR > 2 or (NCCL_MAJOR == 2 and NCCL_MINOR >= 10)):
                return False

    CUDA_MAJOR = int(torch_info['cuda_version'].split('.')[0])
    if (TORCH_MAJOR > 1 or (TORCH_MAJOR == 1 and TORCH_MINOR >= 10)) and (
            CUDA_MAJOR >= 11) and torch.cuda.is_bf16_supported():
        return True
    else:
        return False
